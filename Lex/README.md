...
`parser/` 下为 **LR(1)+LALR(1)** 实验代码：**`grammar.cpp`**、**`seu_lr1_parser.cpp`**（同心项压缩）、入口 **`parser/c99_parse_main.cpp`**、三地址骨架 **`semantic/ir_from_ast.cpp`** + **`parser/c99_ir_main.cpp`**。构建与交付对照见 **`build_parse.cmd`**、**`docs/交付说明.md`**。
